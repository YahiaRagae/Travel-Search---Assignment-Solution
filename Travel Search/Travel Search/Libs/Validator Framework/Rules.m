...
+ (Rule *)checkIfStringIsEmpty:(NSString*) string WithFailureString:(NSString *)failureString withView:(UIView*)view
{
    Rule *resultRule = [[Rule alloc] init];
    resultRule.failureMessage = failureString;
    resultRule.view = view;
    resultRule.isValid = NO ;
    if(string != nil && ![string  isEqual: @"nil"] && [string length] > 0){
        resultRule.isValid = YES ;
    }
    return resultRule;
}
+ (Rule *)checkIfStringIsValidURL:(NSString *)string WithFailureString:(NSString *)failureString withView:(UIView*)view
{
    
    Rule *resultRule = [[Rule alloc] init];
    resultRule.failureMessage = failureString;
    resultRule.view = view;
    resultRule.isValid = [NSString checkIfURL:string];
 
    return resultRule;
}
+ (Rule *)checkIfString:(NSString *)string equalsStr:(NSString*)str WithFailureString:(NSString *)failureString withView:(UIView*)view
{
...
+ (Rule *)checkIfSelectedDistrict:(NSMutableArray *)districts WithFailureString:(NSString *)failureString withView:(UIView*)view{
    Rule *resultRule = [[Rule alloc] init];
    resultRule.failureMessage = failureString;
    resultRule.view = view;
    resultRule.isValid = YES;
    NSPredicate * predicate = [NSPredicate predicateWithFormat:@"distritctIsFilter==true", "\(currentYear)"];
    NSArray * array = [districts filteredArrayUsingPredicate:predicate];
